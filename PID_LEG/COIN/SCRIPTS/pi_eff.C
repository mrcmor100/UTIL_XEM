...
void pi_eff(int RunNumber=0){
  ...
  TString filename = "/net/cdaq/cdaql1data/cdaq/hallc-online-pionLT/ROOTfiles/Analysis/General/Full_Online_coin_replay_11828_300000.root";
  ...
  Long64_t nentries = t->GetEntries();
    ...
    for (Long64_t i=0; i<nentries; i++){
        t->GetEntry(i);
        if(delta > -8. && delta < 8.){
          //Choose pions with NGCER for the calo plots
          if(cer == 0. && etottracknorm > 0.15 && etottracknorm < 0.6){
          hPcal_E_p->Fill(etottracknorm);
          hPpre_E_p->Fill(eprtracknorm);
          hPpre_vs_cal->Fill(etottracknorm, eprtracknorm);
          //Check the impact of different PID legs for the pions  
            if(elreal>0.){
        ...
        //Choose pions with calorimeter for the Cherenkov plots
          ...
          double totpi = hPcal_E_p->GetEntries();
          double totpi_elreal = hPcal_E_p_elreal->GetEntries();
          double totpi_elclean = hPcal_E_p_elclean->GetEntries();
          double totpi_ello = hPcal_E_p_ello->GetEntries();
          double totpi_elhi = hPcal_E_p_elhi->GetEntries();
          double totpi_prlo = hPcal_E_p_prlo->GetEntries();
          double totpi_prhi = hPcal_E_p_prhi->GetEntries();
          double totpi_shlo = hPcal_E_p_shlo->GetEntries();
          
          cout<<"Total Number of Pions:"<<totpi<<" "<<"Total Number of Pions with ELREAL:"<<totpi_elreal<<" "<<"ELREAL PRF:"<<totpi/totpi_elreal<<endl;
          cout<<"Total Number of Pions:"<<totpi<<" "<<"Total Number of Pions with ELCLEAN:"<<totpi_elclean<<" "<<"ELCLEAN PRF:"<<totpi/totpi_elclean<<endl;
          cout<<"Total Number of Pions:"<<totpi<<" "<<"Total Number of Pions with ELLO:"<<totpi_ello<<" "<<"ELLO PRF:"<<totpi/totpi_ello<<endl;
          cout<<"Total Number of Pions:"<<totpi<<" "<<"Total Number of Pions with ELHI:"<<totpi_elhi<<" "<<"ELHI PRF:"<<totpi/totpi_elhi<<endl;
          cout<<"Total Number of Pions:"<<totpi<<" "<<"Total Number of Pions with PRLO:"<<totpi_prlo<<" "<<"PRLO PRF:"<<totpi/totpi_prlo<<endl;
          cout<<"Total Number of Pions:"<<totpi<<" "<<"Total Number of Pions with PRHI:"<<totpi_prhi<<" "<<"PRHI PRF:"<<totpi/totpi_prhi<<endl;
          cout<<"Total Number of Pions:"<<totpi<<" "<<"Total Number of Pions with SHLO:"<<totpi_shlo<<" "<<"SHLO PRF:"<<totpi/totpi_shlo<<endl;
       ...
       leg7->Draw();
       leg8->Draw();
       c1->SaveAs(Form("PID_LEG/HMS/HMS_%d_Total_E_p_with_PID_LEGS_PIONS.pdf",RunNumber));
       ...
       leg15->Draw();
       leg16->Draw();
       c2->SaveAs(Form("PID_LEG/HMS/HMS_%d_PreSh_E_p_with_PID_LEGS_PIONS.pdf",RunNumber));
       ...
       leg18->SetTextColor(kCyan+3);
       leg18->AddEntry(hPcer_cer,"CER","");
       hPcer_cer->Draw("hist same");
       leg17->Draw();
       leg18->Draw();
       c3->SaveAs(Form("PID_LEG/HMS/HMS_%d_NGCER_npe_with_PID_LEGS_PIONS.pdf",RunNumber));
       ...
       leg26->Draw();
       c4->SaveAs(Form("PID_LEG/HMS/HMS_%d_PreSh_vs_Total_E_p_with_PID_LEGS_PIONS.pdf",RunNumber));
       ...
       leg27->Draw();
       leg28->Draw();
       c5->SaveAs("HMS_NGCER_PulseAmp_with_PID_LEGS_PIONS.pdf");
       ...
       leg30->Draw();
       leg31->Draw();
       c6->SaveAs("HMS_PreSh_PulseAmp_with_PID_LEGS_PIONS.pdf");
*/
}
