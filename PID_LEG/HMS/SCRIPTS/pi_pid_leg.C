...
void pi_pid_leg(int RunNumber=0){
   if (RunNumber == 0){
  ...
  Long64_t nentries = t->GetEntries();
    ...
    for (Long64_t i=0; i<nentries; i++){
        ...
        //Choose pions with calorimeter for the Cherenkov plots
          if(etottracknorm < 0.6 && etottracknorm > 0.15){
            hPcer->Fill(cer);
            if(cer>0){
              hPcer_cer->Fill(cer);}
            if(cer_mult>0){
       ...
       leg1->AddEntry(hPcal_E_p,"3/4-PIONS","");
       ...
       TLegend *leg9 = new TLegend(0.7,0.2,.9,0.55);
       leg9->SetFillStyle(0);
       ...
       leg8->Draw();
       leg9->Draw();
       c1->SaveAs(Form("PID_LEG/HMS/SHMS_%d_Total_E_p_with_PID_LEGS_PIONS.pdf",RunNumber));
       ...
       leg10->AddEntry(hPpre_E_p,"3/4-PIONS","");
       ...
       TLegend *leg18 = new TLegend(0.7,0.2,.9,0.55);
       ...
       leg17->Draw();
       leg18->Draw();
       c2->SaveAs(Form("PID_LEG/HMS/SHMS_%d_PreSh_E_p_with_PID_LEGS_PIONS.pdf",RunNumber));
       ...
       leg19->AddEntry(hPcer,"3/4-PIONS","");
       hPcer->SetStats(0);
       hPcer->Draw("hist");
       ...
       leg19->Draw();
       leg20->Draw();
       c3->SaveAs(Form("PID_LEG/HMS/SHMS_%d_NGCER_npe_with_PID_LEGS_PIONS.pdf",RunNumber));
       ...
       leg21->AddEntry(hPpre_vs_cal,"3/4-PIONS","");
       ...
       c4->cd(9);
       gPad->SetLogz();
       TLegend *leg29 = new TLegend(0.15,0.7,.35,0.85);
       ...
       hPpre_vs_cal_cer->Draw("colz");
       leg29->Draw();
       c4->SaveAs(Form("PID_LEG/HMS/SHMS_%d_PreSh_vs_Total_E_p_with_PID_LEGS_PIONS.pdf",RunNumber));
       ...
       leg27->AddEntry(hPcer_amp,"3/4-PIONS","");
       ...
       leg29->AddEntry(hPpre_amp,"3/4-PIONS","");
